...
protected:
    ...
    void agregar_depedencias();
    void crear_doble_dependencia();
    void eliminar_dependencias();
    ...
    Gtk::Button boton_crear_doble_dependencia;
...
VentanaPrincipal::VentanaPrincipal() : tareas(nullptr), cantidadTareas(0) {
    ...
    boton_agregar_dependencias.set_label("Crear dependencia");
    boton_agregar_dependencias.set_size_request(800,50);
    boton_crear_doble_dependencia.set_label("Crear doble dependencia");
    boton_crear_doble_dependencia.set_size_request(800,50);
...
void VentanaPrincipal::agregar_depedencias() {
    Glib::ustring nombre_origen_dependencia = entrada_origen_dependencia.get_text();
    Glib::ustring nombre_destino_dependencia = entrada_destino_dependencia.get_text();

    if (nombre_origen_dependencia.empty() || nombre_destino_dependencia.empty()) {
        auto dialog = new Gtk::MessageDialog(*this, "Error: Completa todos los campos");
        dialog->set_secondary_text("Por favor, asegúrese de ingresar el origen y destino de la dependencia.");
        dialog->set_modal(true);
        dialog->signal_response().connect([dialog](int response_id) {
            dialog->hide();
            delete dialog;
    ...
    int id_origen = -1, id_destino = -1;
    for (int i = 0; i < cantidadTareas; ++i) {
        if (tareas[i]->getNombre() == origen) {
            id_origen = i;
        }
        if (tareas[i]->getNombre() == destino) {
            id_destino = i;
        }
    }
    ...
    try {
        grafo.agregarDependencia(id_origen, id_destino);
        grafo.mostrarGrafo();
        auto dialog = new Gtk::MessageDialog(*this, "¡Dependencia agregada con éxito!");
        dialog->set_secondary_text("Se ha creado la dependencia correctamente entre las tareas.");
        dialog->set_modal(true);
        dialog->signal_response().connect([dialog](int response_id) {
            dialog->hide();
            delete dialog;
        ...
        dialog->show();
        entrada_origen_dependencia.set_text("");
        entrada_destino_dependencia.set_text("");

    } catch (const std::exception& e) {
        auto dialog = new Gtk::MessageDialog(*this, "Error al agregar dependencia");
        dialog->set_secondary_text(e.what());
        dialog->set_modal(true);
        dialog->signal_response().connect([dialog](int response_id) {
            dialog->hide();
...
void VentanaPrincipal::eliminar_dependencias() {
    ...
    try {
        grafo.eliminarDependencia(id_origen, id_destino);
        grafo.mostrarGrafo();
        auto dialog = new Gtk::MessageDialog(*this, "¡Dependencia eliminada con éxito!");
        dialog->set_secondary_text("Se ha eliminado la dependencia correctamente entre las tareas.");
        dialog->set_modal(true);
        dialog->signal_response().connect([dialog](int response_id) {
            dialog->hide();
            delete dialog;
        ...
        auto dialog = new Gtk::MessageDialog(*this, "Error al eliminar dependencia");
        dialog->set_secondary_text(e.what());
        dialog->set_modal(true);
        dialog->signal_response().connect([dialog](int response_id) {
            dialog->hide();
...
void VentanaPrincipal::crear_doble_dependencia() {
    ...
    try {
        ...
        grafo.agregarDependencia(id_origen, id_destino);
        grafo.agregarDependencia(id_destino, id_origen);

        grafo.mostrarGrafo();
        auto dialog = new Gtk::MessageDialog(*this, "¡Dependencia doble creada con éxito!");
        dialog->set_secondary_text("Se ha creado una dependencia bidireccional entre las tareas.");
        dialog->set_modal(true);
        dialog->signal_response().connect([dialog](int response_id) {
            dialog->hide();
            delete dialog;
        ...
        auto dialog = new Gtk::MessageDialog(*this, "Error al crear dependencia doble");
        dialog->set_secondary_text(e.what());
        dialog->set_modal(true);
        dialog->signal_response().connect([dialog](int response_id) {
            dialog->hide();
